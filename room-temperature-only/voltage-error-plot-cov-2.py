...
for i in range(n_param):
    for j in range(n_param):
        if i == j:
            ...
            px_e = param_exp[:, j]
            py_e = param_exp[:, i]
            axes[i, j].scatter(px_e, py_e, c='#1f77b4', label='Data')

            px_s = param_syn[:, j]
            py_s = param_syn[:, i]
            axes[i, j].scatter(px_s, py_s, c='#d62728', label='Syn. voltage offset')
...
plt.savefig('%scov-plot-2.png' % saveas, bbox_inch='tight', dpi=300)
...
plt.savefig('%scov-plot-2-cbox.png' % saveas, bbox_inch='tight', dpi=100)
plt.savefig('%scov-plot-2-cbox.pdf' % saveas, format='pdf', bbox_inch='tight')
...
plt.savefig('%scov-plot-2-cbox2.png' % saveas, bbox_inch='tight', dpi=100)
plt.savefig('%scov-plot-2-cbox2.pdf' % saveas, format='pdf', bbox_inch='tight')
