...
for cell in cells_kylie:
    ...
    all_param.append(obtained_parameters)
    all_solutions.extend(obtained_parameters)
    cells_label.extend([r'Manual patch'] * len(labels))
    all_labels.extend(labels)
...
sns.violinplot(x=all_labels, y=np.log10(all_solutions), hue=cells_label,
              zorder=1)
''' # Probably don't need to plot mean, violin plot has it already
plt.plot(np.log10(mean_param), c='b', ls='', marker='x',
         label=r'Automated patch mean', zorder=2)
plt.plot(np.log10(kylie_mean_param), c='r', ls='', marker='s',
         label=r'Manual patch mean', zorder=2)
'''
plt.legend(fontsize=8)
plt.ylabel(r'$\log_{10}($Value$)$', fontsize=32)
# plt.xlabel('Parameter', fontsize=32)
plt.xticks(fontsize=32)
plt.yticks(fontsize=24)
plt.savefig('./figs/paper/violinplot.png', bbox_inches='tight', dpi=300)
plt.savefig('./figs/paper/violinplot.pdf', format='pdf', bbox_inches='tight')
