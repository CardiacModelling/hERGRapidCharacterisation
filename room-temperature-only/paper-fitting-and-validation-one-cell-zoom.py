...
for i in range(int(len(protocol_list) / 3)):
    ...
    axes[3, i] = fig.add_subplot(grid[n_shift+0:n_shift+4, i_grid:f_grid])
    axes[3, i].set_xticklabels([])
    axes[4, i] = fig.add_subplot(grid[n_shift+4:n_shift+11, i_grid:f_grid])
    axes[5, i] = np.empty(n_maxzoom, dtype=object)

    # Third 'row'
    n_shift = 2 * (n_ygrid + bigygap)
    axes[6, i] = fig.add_subplot(grid[n_shift+0:n_shift+4, i_grid:f_grid])
    axes[6, i].set_xticklabels([])
    axes[7, i] = fig.add_subplot(grid[n_shift+4:n_shift+11, i_grid:f_grid])
    axes[8, i] = np.empty(n_maxzoom, dtype=object)
...
axes[5, 0][0].set_ylabel('Zoom in', fontsize=14)
...
for i_prt, prt in enumerate(protocol_list):
    ...
    if prt not in protocol_iv:
        ...
        # simulation
        if prt == 'staircaseramp':
            axes[ai + 1, aj].plot(times, simulation, lw=2, c='#d62728',
                                  label='model fit to data')
        else:
            axes[ai + 1, aj].plot(times, simulation, lw=2, c='#d62728',
    ...
    if prt not in protocol_iv:
        ...
        for i_z, (t_i, t_f) in enumerate(zoom_in_win[prt]):
            ...
            axes[ai + 2, aj][i_z].plot(times[idx_i:idx_f],
                    zoom_in_segment_data, lw=2, alpha=0.5, c='#1f77b4')
            axes[ai + 2, aj][i_z].plot(times[idx_i:idx_f],
                    zoom_in_segment_sim, lw=2, c='#d62728')
            ...
            axes[ai + 2, aj][i_z].arrow(1, -0.05, -1, 0,
                    ...
                    fc='k', ec='k', transform=axes[ai + 2, aj][i_z].transAxes)
...
axes[1, 0].legend()
axes[1, 1].legend()
grid.tight_layout(fig, pad=0.6, rect=(0, 0.04, 1, 1))
grid.update(wspace=20, hspace=0.0)
plt.savefig('%s/fitting-and-validation-%s_%s-zoom.png' % (savedirlr, \
