...
# Plot
fig, axes = plt.subplots(2, 2, figsize=(7.5, 6.75))
axes[0, 0].set_ylabel(r'$a_\infty$', fontsize=14)
axes[0, 1].set_ylabel(r'$\tau_a$ [ms]', fontsize=14)
...
for i_T, T in enumerate(temperatures):
    ...
    # Plot
    axes[0, 0].plot(v, a_inf_hbm, ls='-', lw=1.5, c=colour_list[i_T],
            label='_nolegend_' if i_T else 'HBM')

    axes[0, 1].plot(v, a_tau_hbm, ls='-', lw=1.5, c=colour_list[i_T])

    axes[1, 0].plot(v, r_inf_hbm, ls='-', lw=1.5, c=colour_list[i_T])
...
plt.savefig('%s/ss-tau.pdf' % (savedir), format='pdf', bbox_iches='tight')
