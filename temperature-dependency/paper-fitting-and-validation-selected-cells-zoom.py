...
cell_ranking_file = './manualselection/paper-rank-%s.txt' % file_id

ref_norm = {
        'herg25oc': ('herg25oc1', 'D19'),
        'herg27oc': ('herg27oc1', 'D13'),
        'herg30oc': ('herg30oc1', 'D11'),
        'herg33oc': ('herg33oc1', 'D23'),
        'herg37oc': ('herg37oc3', 'D13'),
        }
ref_ylim = {
        'herg25oc': [(-900, 600), (-20, 600)],
        'herg27oc': [(-900, 600), (-20, 600)],
        'herg30oc': [(-900, 600), (-20, 600)],
        'herg33oc': [(-950, 850), (-20, 850)],
        'herg37oc': [(-1200, 1100), (-50, 1100)],
        }
...
for i in range(int(len(protocol_list) / 3)):
    ...
    if i == 0:
        ...
        axes[0, i].set_xticklabels([])
        axes[1, i] = fig.add_subplot(grid[11:21, i_grid:f_grid])
...
colour_list_d = sns.color_palette('Blues_d', n_colors=len(RANKED_CELLS))
colour_list_d = colour_list_d.as_hex()[::-1]
colour_list_s = sns.color_palette('Oranges', n_colors=len(RANKED_CELLS))
...
for i_prt, prt in enumerate(protocol_list):
    ...
    if prt == 'staircaseramp':
        times = np.loadtxt('%s/%s-%s-times.csv' % (data_dir_staircase,
            file_id + '1', prt), delimiter=',', skiprows=1)
    else:
        times = np.loadtxt('%s/%s-%s-times.csv' % (data_dir, file_id + '1',
    ...
    if norm_method == 1 and isNorm:
        ref_file = ref_norm[file_id][0]
        ref_cell = ref_norm[file_id][1]
        if prt == 'staircaseramp':
        ...
        if prt not in protocol_iv:
            ...
            '''
            if prt == 'staircaseramp':
                minimum, maximum = ref_ylim[file_id][0]
            else:
                minimum, maximum = ref_ylim[file_id][1]
            for i in range(2):
    ...
    for i_CELL, CELL in enumerate(RANKED_CELLS):
        ...
        if prt not in protocol_iv:
            for i_z, (t_i, t_f) in enumerate(zoom_in_win[prt]):
                ...
                if i_CELL == 0:
                    top_v = [(times[idx_i], minimum), (times[idx_f], minimum)]
...
plt.savefig('%s/fitting-and-validation-%s-zoom.png' % (savedirlr, file_id),
