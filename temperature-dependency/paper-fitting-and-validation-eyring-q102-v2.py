...
q10_2mean = np.copy(q10_mean)
hbm_T_mean_q10 = transform_to_model_param(
        np.mean(mean_chains[-1], axis=0))
for i in [2, 4, 6, 8]:
    q10_2mean[i, 0] = hbm_T_mean_q10[i]
...
for i_T, (file_name, T) in enumerate(zip(file_list_tmp, temperatures)):
    ...
    # Q10 parameters
    q10_T_2mean = q10T(q10_2mean, T)
    q10_2model_param = eyring_transform_to_model_param(q10_T_2mean, T)
    q10_2sim = model.simulate(q10_2model_param, times_sim)
    ...
    if norm_method == 1:
        ...
        res_q = minimize(lambda x: np.sum(
                np.abs(q10_2sim / x - ref_data)), x0=norm_sim)
        ...
        if prt == 'staircaseramp':
            norm_q10 = est_g_staircase(q10_2sim, times_sim, p0=[800, 0.025],
                                        debug=False)
    ...
    if prt in protocol_iv:
        ...
        iv_i = protocols.get_corrected_iv(eyring_sim, times,
                                          *protocol_iv_args[prt]())
        axes[ai, aj].plot(iv_v, iv_i / np.max(iv_i), lw=1, alpha=1,
                          ls=':', c='C2', zorder=2, label='Eyring mean')
    else:
        axes[ai, aj].plot(times_sim,
                          eyring_sim / norm_eyring,
                          alpha=1, lw=1, ls=':', c='C2', zorder=2,
                          label='Eyring mean')

    # Q10
    if prt in protocol_iv:
        q10_2sim, t = protocol_iv_convert[prt](q10_2sim, times_sim)
        assert(np.mean(np.abs(t - times)) < 1e-8)
        iv_v = protocol_iv_v[prt]() * 1000  # mV
        iv_i = protocols.get_corrected_iv(q10_2sim, times,
                                          *protocol_iv_args[prt]())
        axes[ai, aj].plot(iv_v, iv_i / np.max(iv_i), lw=1, alpha=1,
                          ls='--', c='C3', zorder=3, label=r'Q$_{10}$ mean')
    else:
        axes[ai, aj].plot(times_sim,
                          q10_2sim / norm_q10, ls='--',
    ...
    if prt not in protocol_iv:
        for i_z, (t_i, t_f) in enumerate(zoom_in_win[prt][0]):
            ...
            zoom_in_segment_q10 = q10_2sim[idx_i:idx_f]
            # Plot
...
plt.savefig('%s/fitting-and-validation-eyring-q102-%s-v2.png' % (savedirlr,
            prt), bbox_inch='tight', pad_inches=0, dpi=300)
plt.savefig('%s/fitting-and-validation-eyring-q102-%s-v2.png' % (savedir, prt),
